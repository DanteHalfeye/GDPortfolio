...
Players control an **asteroid** through increasingly difficult rooms filled with enemies attempting to shoot them down. Instead of relying primarily on traditional weapons, the player's **dash is their main offensive tool**, allowing them to collide with enemies and destroy them.
...
* Iterating on movement to make combat feel responsive.
...
The project gave me the opportunity to design and program an asteroid controller where movement is directly tied to combat, while also building a room-based progression system that gradually increases the challenge and rewards the player with new upgrades.